"use client";
import Image from "next/image";
import Link from "next/link";

const SidebarClient = () => {
  return (
    <>
      <aside
        className="
          fixed left-0 top-0 w-64 h-screen bg-background text-azul-kurve flex flex-col py-8 px-4
          transition-all duration-300 overflow-y-auto"
      >
        <Image
          src="/kurve-icon.png"
          alt="Logo"
          width={100}
          height={100}
          priority
          className="mb-6 ml-3"
        />
        <div className="mb-6 p-4 bg-verde-kurve-light rounded-lg border border-verde-kurve/20">
          <p className="text-xs font-bold text-verde-kurve uppercase tracking-wide mb-2">
            Cliente
          </p>
          <h3 className="text-sm font-bold text-foreground mb-2">
            Estudio Norte
          </h3>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-verde-kurve"></div>
            <p className="text-xs text-gris-kurve-dark">Plan activo · Mayo</p>
          </div>
        </div>

        <nav className="sticky flex flex-col gap-4 ml-3"></nav>
        <nav className="sticky flex flex-col gap-4 ml-3">
          <div>
            <p className="uppercase font-bold text-[13px] mb-2 text-gris-kurve-dark">
              Tu cuenta
            </p>
            <div className="flex flex-col">
              <Link
                href="/client/consumo"
                className="py-2 pl-2 rounded-lg hover:bg-muted transition-colors"
              >
                Resumen de consumo
              </Link>
              <Link
                href="/client/piezas"
                className="py-2 pl-2 rounded-lg hover:bg-muted transition-colors"
              >
                Piezas entregadas
              </Link>
              <Link
                href="/client/equipo"
                className="py-2 pl-2 rounded-lg hover:bg-muted transition-colors"
              >
                Equipo asignado
              </Link>
            </div>
          </div>
          <div>
            <p className="uppercase font-bold text-[13px] mb-2 text-gris-kurve-dark">
              Recursos
            </p>
            <div className="flex flex-col">
              <Link
                href="/client/links-docs"
                className="py-2 pl-2 rounded-lg hover:bg-muted transition-colors"
              >
                Links y documentos
              </Link>
              <Link
                href="/client/estado"
                className="py-2 pl-2 rounded-lg hover:bg-muted transition-colors"
              >
                Estado de cuenta
              </Link>
            </div>
          </div>
        </nav>

        <div className="mt-auto pt-4">
          <div className="flex items-center gap-3 p-3 rounded-lg border border-gris-kurve-light hover:bg-muted transition-colors cursor-pointer">
            {/* Avatar */}
            <div className="w-10 h-10 rounded-full bg-verde-kurve-dark flex items-center justify-center text-white font-bold text-sm">
              MV
            </div>

            {/* User Info */}
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-foreground truncate">
                Martin V.
              </p>
              <p className="text-xs text-gris-kurve-dark truncate">Cliente</p>
            </div>

            {/* Menu Icon */}
            <button className="text-gris-kurve-dark hover:text-foreground transition-colors cursor-pointer">
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                className="hover:scale-130 hover:rotate-5 transition-transform"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M5 4.75C5 3.78 5.78 3 6.75 3H13.25C14.22 3 15 3.78 15 4.75V19.25C15 20.22 14.22 21 13.25 21H6.75C5.78 21 5 20.22 5 19.25V4.75Z"
                  stroke="#1F3B45"
                  strokeWidth="2"
                  strokeLinejoin="round"
                />

                <path
                  d="M11 12H21"
                  stroke="#76B041"
                  strokeWidth="2"
                  strokeLinecap="round"
                />

                <path
                  d="M17 8L21 12L17 16"
                  stroke="#76B041"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />

                <path
                  d="M8 8H11"
                  stroke="#1F3B45"
                  strokeWidth="2"
                  strokeLinecap="round"
                />
              </svg>
            </button>
          </div>
        </div>
      </aside>
    </>
  );
};

export default SidebarClient;
